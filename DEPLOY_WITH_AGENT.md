...
Read README.md, docs/WALKTHROUGH.md, and docs/SETUP_NOTES.md first. Work in this repository and use its pinned CLI tools.
...
3. Inspect `pnpm exec stripe projects status`. If uninitialized, use the walkthrough's manual initialization command. Let the owner complete browser authentication. Do not bypass identity guards or repeatedly overwrite a session on a connectivity error.
4. Inspect the live Cloudflare catalog. Use Projects' Cloudflare link flow to authorize an existing account or create one if needed. Review terms and paid changes with the owner.
...
6. Pull the active environment. Run `pnpm configure` to read its output directly. If the provider uses an unsupported key shape, inspect names without printing secrets and update the resolver with a focused test. Do not ask the user to copy API keys as the normal flow.
